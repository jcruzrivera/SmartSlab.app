...
export function PieceEditor({
  ...
  const addPiece = useCallback(() => {
    setPieces((prev) => [
      ...prev,
      toPieceRow({ label: "", widthIn: 0, heightIn: 0 }),
    ]);
  }, []);
    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Review the plan and confirm each piece. SmartFinder will search your
          inventory first, then the marketplace.
        </p>
      </div>
            ...
            <div className="flex items-start gap-3 rounded-2xl border border-brand/30 bg-brand/10 p-4 text-sm">
              ...
              <p className="text-slate-700 dark:text-slate-200">
                <span className="font-semibold">
                  Auto-filled from your plan.
                </span>{" "}
                Compare each piece with the preview and adjust labels or
                dimensions before searching — AI estimates can be imperfect.
              </p>
            </div>
          ) : null}
