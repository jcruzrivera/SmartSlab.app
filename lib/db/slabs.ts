...
export async function listPublicSlabsByIds(
  ...
  if (!isDbConfigured() || ids.length === 0) {
    return [];
  }
  ...
  const rows = await db.query.slabs.findMany({
    where: and(
      inArray(slabs.id, ids),
      inArray(slabs.status, ["available", "reserved"]),
    ),
    with: slabRelations,
  });
  ...
  return ids
    .map((id) => byId.get(id))
    .filter((row): row is NonNullable<typeof row> => Boolean(row)) as SlabWithRelations[];
}
