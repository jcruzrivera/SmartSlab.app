 ...
 * Configure with:
 *   RESEND_API_KEY=re_********************
 *   EMAIL_FROM="SmartSlab <orders@yourdomain.com>"   (optional; defaults to Resend's sandbox sender)
 *   PLATFORM_NOTIFICATION_EMAIL=ops@yourdomain.com    (optional; copy of every order)
 *
 * Every send is best-effort: if the key is missing or the request fails, we log
 * and continue so a mail hiccup never blocks payment fulfillment.
 */
...
/**
 * Sends the three order emails (buyer, vendor, platform). Idempotency is the
 * caller's responsibility (only call once per transaction transition to paid).
 */
export async function sendPaymentNotifications(
  ...
  // Buyer: confirmation + the now-unlocked vendor contact details.
  await sendEmail({
    to: data.buyer.email,
    ...
    html: layout(
      ...
      `<p>Hi ${buyerName}, your payment of <strong>${money(data.total)}</strong> for
       ...
       <p>You can now coordinate pickup/delivery with the vendor:</p>
       <ul>
  ...
  // Vendor: you sold a slab + buyer contact + payout.
  await sendEmail({
  ...
  // Platform: internal copy of every order.
